  ...
  return (
    <>
      ...
      <Paragraph>
        ...
        <Math latex="p_t" /> attains a stationary distribution in the limit. The
        authors also mentions the time reversal process{" "}
        <Math latex="q_t=p_{T-t}" /> at time being described by
        <Math display={true} latex="\frac{dq_t}{dt}=R_{t}q_t" />
        where the matrix <Math display={false} latex="R_{t}" /> has off-diagonal
        components in row <Math latex="x" /> and column
        <Math latex="y" />
      ...
      <Paragraph>
        ...
        in the continuous diffusion process, the ratio
        <Math latex="\frac{p_t(x)}{p_t(y)}" /> occuring in the matrix{" "}
        <Math latex="R_t" /> above are called <Text as="b">concrete score</Text>
        . The neural network learns the concrete score. In the the first
        author&apos;s blog, another motivation for learning the concrete score
        is given: a nerual network <Math latex="f_\theta" /> can parametrize the
        probability distribution as in an energy based model{" "}
        <Math latex="p_\theta(x)=e^{f_\theta(x)}/\mathcal{Z}" />. The partition
        function <Math latex="\mathcal{Z}" /> is intractable, so a better idea
        is to work with the ratios, which is the concrete score
        <Math
          latex="\frac{p_t(x)}{p_t(y)}=\frac{e^{f_\theta(x)}/\mathcal{Z}}{e^{f_\theta(y)}/\mathcal{Z}}"
          display={true}
        />
      </Paragraph>

      <Paragraph>
        After the author considers why previous approaches lead to
        underperforming models, they propose the{" "}
        <Text as="b">score entropy</Text>
        <Math
          display={true}
          latex="\mathcal{L}_{\text{SE}}=\mathbb{E}_{x\sim p}\left[\, \sum_{y, y\neq x} w_{xy}\left(s_\theta(x)_y-\frac{p(y)}{p(x)}\log s_\theta(x)_y + K\left(\frac{p(y)}{p(x)}\right)\right)\right]"
        />
        for a function <Math latex="K(x)=x(\log x - 1)" />. This loss is
        inspired by the Bregman divergence{" "}
        <Math latex="D_F\left(s_\theta(x)_y, \frac{p(x)}{p(y)}\right)" /> where{" "}
        <Math latex="F(x)=-\log(x)" />. Thus the score entropy is{" "}
        <Text as="u">non-negative</Text>, <Text as="u">symmetric</Text>, and{" "}
        <Text as="u">convex</Text>.
      </Paragraph>
